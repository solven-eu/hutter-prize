package eu.solven.hutter_prize.reversible.lstm;
...
public final class TrainMnistWithLSTM {
	...
	public static TrainingResult runExample(String[] args) throws IOException, TranslateException {
		...
		try (Model model = Model.newInstance("lstm")) {
			...
			try (Trainer trainer = model.newTrainer(config)) {
				...
				/*
				 * MNIST is 28x28 grayscale image and pre processed into 28 * 28 NDArray. 1st axis is batch axis, we can
				 * use 1 for initialization.
				 */
	...
	private static Block getLSTMModel() {
		...
		block.add(new LSTM.Builder().setStateSize(64).setNumLayers(1).optDropRate(0).optReturnState(false).build());
	...
	public static DefaultTrainingConfig setupTrainingConfig(Arguments arguments) {
		...
		return new DefaultTrainingConfig(Loss.softmaxCrossEntropyLoss()).addEvaluator(new Accuracy())
